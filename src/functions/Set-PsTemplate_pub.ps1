#0001,Set-PsTemplate_pub.ps1,00.00.01,2016-06-17 20:00:00,Joseph Pulk$
function Set-PsTemplate{
    <#
    ...
    BEGIN{
        if($version){
            ...
            Return $ver        }    }    PROCESS{      if($name){        if($script:MasterRepoInfo.userPreferences.Templates.Name -eq $name){          ($script:MasterRepoInfo.userPreferences.Templates | ?{$_.Name -eq $name}).module = $Module          return $script:MasterRepoInfo.userPreferences.Templates        }      }          }}

if ($MyInvocation.ScriptName -match '.psm1$') {
    Export-ModuleMember -Function Set-PsTemplate
}